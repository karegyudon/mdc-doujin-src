...
def get_data_from_json(file_number, oCC):
    ...
    """
        ...
        # 使用当前目录下的MappingTable文件夹
    import os
    current_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    actor_mapping_path = os.path.join(current_dir, 'MappingTable', 'mapping_actor.xml')
    info_mapping_path = os.path.join(current_dir, 'MappingTable', 'mapping_info.xml')
    
    # 如果本地文件存在就使用本地文件，否则使用默认路径
    if os.path.exists(actor_mapping_path):
        actor_mapping_data = etree.parse(actor_mapping_path)
    else:
        actor_mapping_data = etree.parse(str(Path.home() / '.local' / 'share' / 'mdc' / 'mapping_actor.xml'))
    
    if os.path.exists(info_mapping_path):
        info_mapping_data = etree.parse(info_mapping_path)
    else:
    ...
    if "rj" in lo_file_number or "vj" in lo_file_number or re.search(r"[぀-ゟ゠-ヿ]+", file_number):
        sources = ["dlsite"]
        print("[+]DEBUG-detector: dlsite -" , lo_file_number)
    elif "GETCHU" in file_number.upper() or (lo_file_number.startswith("item") and re.match(r"item\d+$", lo_file_number)):
        sources = ["getchu"]
        print("[+]DEBUG-detector: getchu -" , lo_file_number)
    elif "d_" in lo_file_number : 
        print("[+]DEBUG-detector: fanza -" , lo_file_number)    
        sources = ["fanza"]
    elif re.match(r"[a-zA-Z]+-\d{3}", file_number):
        # 格式要求：英文字符开头，后续一个"-"号，再加上3位数字
        sources = ["jav321"]
        print("[+]DEBUG-detector: jav321 -" , lo_file_number)
    else:
        print("[!] 无法识别文件名格式，没有找到匹配的数据源")
        return {}
    
    # print("[+]DEBUG-detector:", sources)
